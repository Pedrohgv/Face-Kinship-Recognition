...
from config import number_train_families
...
def process_data(set_seed=False, seed=1):
    ...
    train_positive_relations = positive_relations[relations_families.isin(
        train_families.values)]
    test_positive_relations = positive_relations[relations_families.isin(
        test_families.values)]
...
def check_families(positive_relations):
    ...
    positive_relations = positive_relations.drop(
        labels=indexes_to_remove, axis=0)
...
def get_triplet_img_folders(families, positive_relations, set_seed=False, seed=1):
    ...
    if set_seed == True:
        positive_pair = positive_relations.sample(
            n=1).sample(n=2, axis=1, random_state=seed)
    else:
    ...
    random_person = random_family + '/' + \
        random.choice(os.listdir('train/' + random_family))
...
def get_triplet_img_paths(families, positive_relations):
    ...
    anchor, positive, negative = get_triplet_img_folders(
        families, positive_relations)
...
def make_triplet_generator(families, positive_relations):

    # =============================================================================
    #     Generator that returns a random anchor, positive and negative images each time it is called
    # =============================================================================
    while True:

        # generates random triplet
        path_anchor_img, path_positive_img, path_negative_img = get_triplet_img_paths(
            families, positive_relations)

        # loads and preprocess the images to be used in the in the algorithm
        anchor_img = pre_processing(cv2.imread(path_anchor_img))
        positive_img = pre_processing(cv2.imread(path_positive_img))
        negative_img = pre_processing(cv2.imread(path_negative_img))

        # 0 is just a dummy 'y_true' value, since true labels aren't required in triplet loss
        yield ({'Anchor_input': anchor_img, 'Positive_input': positive_img, 'Negative_input': negative_img}, [1, 0, 0, 0])
...
def make_triplet_dataset(families, positive_relations):
    ...
    dataset = Dataset.from_generator(lambda: make_triplet_generator(families, positive_relations),
                                     output_types=({'Anchor_input': tf.float32, 'Positive_input': tf.float32, 'Negative_input': tf.float32}, tf.int64))
