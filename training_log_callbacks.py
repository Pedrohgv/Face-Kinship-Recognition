...
# =============================================================================
    ...
    def on_train_end(self, logs={}):
        ...
        log_series['training_time'] = '{:02}:{:02}:{:02}'.format(int(hours), int(minutes), int(seconds))
        
        # saves the training data to a csv file
        log_series.to_csv(self.folder_path + '/' + 'Training logs.csv', header = False)
            ...
            for window, plot_settings in zip(self.windows, self.plots_settings):
                ...
                if (plot_settings['last_50'] == False) or (epoch < 50):
                    ...
                    for variable, legend in plot_settings['variables'].items():
                        window.plot(self.losses_and_metrics_dict[variable], label = legend)
                    ...
                    window.set_title(plot_settings['title'])
                     ...
                     # plots all the variables for this plot
                    for variable, legend in plot_settings['variables'].items():
                        window.plot(range(epoch - 50, epoch), self.losses_and_metrics_dict[variable][-50:], label = legend)
            
                    window.set(xlabel = 'Last 50 Epochs', ylabel = plot_settings['ylabel'])
                    window.set_title(plot_settings['title'] + ' on last 50 epochs')
